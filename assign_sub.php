...
<body>
    ...
    <?php
    $query = "SELECT subject_std.*, subjects.subjects As sub_name, standards.std_name AS std_name
    FROM `subject_std` 
    JOIN subjects ON subjects.id = subject_std.sub_id
    JOIN standards ON standards.id = subject_std.std_id ";
    ...
    <?php
        if (isset($_POST['submitsub'])) {
            ...
            $assign_chap_query = "insert into subject_std (sub_id, std_id) values ('$subject_name', '$standard_name')";
    ...
    <form action="" method="post">
        ...
        <select name="studentname">
            ...
            From `user` 
            join user_type on user_type.user_id = user.Id 
            Where user_type.access_id = 3";
            $result = mysqli_query($conn, $query);
    ...
    <br>
        ...
        if (isset($_POST['submituser'])) {
            ...
            $result = mysqli_query($conn, $assign_chap_query);
            if ($result) {
                $message = 'Student is assign to standard.';
                echo '<script type="text/javascript">alert("' . $message . '");
           window.location.href="assign_sub.php";</script>';
            } else {
                $message = 'Studentt is not assign to standard.';
                echo '<script type="text/javascript">alert("' . $message . '");
           window.location.href="assign_sub.php";</script>';
            }
        }
